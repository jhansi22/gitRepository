...
function setTaskTitle() {
	...
	taskTitle.value = formNum.value +": "+ formTitle.value;
}
...
function expandTextArea(oTextArea) {
    ...
    if (navigator.appName.indexOf("Microsoft") == -1) {
        ...
        // one line paragraph without spaces
        if (nTextLength > 0 && oTextArea.scrollWidth > textAreaWidth) {
            var nChars = 80;
            var aLine = oTextArea.value.match(new RegExp('.{1,' + nChars + '}', 'g'));
            oTextArea.value = aLine.join("\n");
        }

        if (nSpaceCount == nEnterCount && oTextArea.scrollWidth > textAreaWidth) {
